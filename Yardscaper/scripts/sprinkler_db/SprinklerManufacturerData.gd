...
static func from_dict(manu_name: String, data: Dictionary) -> SprinklerManufacturerData:
	...
	for head_name in head_datas.keys():
		var head_data := head_datas[head_name] as Dictionary
		out._heads_by_name[head_name] = SprinklerHeadData.from_dict(head_name, head_data)
	return out

func get_head_count() -> int:
	return _heads_by_name.size()

func get_head_names() -> Array:
	return _heads_by_name.keys()
func get_head(head_name: String) -> SprinklerHeadData:
	if head_name in _heads_by_name:
		return _heads_by_name[head_name]
	return null
