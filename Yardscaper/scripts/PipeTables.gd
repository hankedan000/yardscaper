...
const MINOR_LOSSES_LUT = [
	# PVC minor losses
	{
		...
		&'loss_by_fitting' : [
			{
				&'fitting' : FittingType.COUPLING,
				&'loss_by_diam' : [
					[0.5,1.0], [0.75,1.5], [1.0,2.0], [1.25,2.8], [1.5,3.5], [2.0,4.5], [2.5,5.5], [3.0,6.5], [4.0,9.0], [6.0,14.0]
				]
			},
			{
			...
			{
				&'fitting' : FittingType.TEE_BRANCH_OUT,
				&'loss_by_diam' : [
			...
			{
				&'fitting' : FittingType.TEE_BRANCH_IN,
				&'loss_by_diam' : [
					[0.5,3.1], [0.75,4.1], [1.0,5.3], [1.25,6.9], [1.5,8.1], [2.0,10.3], [2.5,12.3], [3.0,16.3], [4.0,20.1], [5.0,25.2], [6.0,30.3], [8.0,40.0], [10.0,50.1], [12.0,58.0]
				]
			}
...
enum LossLookupError {
	# OOR - Out Of Range
	OK, MaterialOOR, FittingOOR, DiameterOOR, EmptyLossTable
}
...
static func lookup_minor_loss(material: MaterialType, fitting: FittingType, diam_ft: float) -> LossLookupResult:
	if fitting == FittingType.NONE:
		return LossLookupResult.from_ok_value(0.0)
	
	var diam_in := Utils.ft_to_inches(diam_ft)
	for mat_entry in MINOR_LOSSES_LUT:
		if mat_entry[&'material'] == material:
			return _lookup_minor_loss_in_mat_table(mat_entry, fitting, diam_in)
	return LossLookupResult.from_error(LossLookupError.MaterialOOR)

static func _lookup_minor_loss_in_mat_table(mat_entry: Dictionary, fitting: FittingType, diam_in: float) -> LossLookupResult:
	var fitting_lut := mat_entry[&'loss_by_fitting'] as Array
	for fitting_enty in fitting_lut:
		if fitting_enty[&'fitting'] == fitting:
			return _lookup_minor_loss_in_fitting_table(fitting_enty, diam_in)
	return LossLookupResult.from_error(LossLookupError.FittingOOR)

static func _lookup_minor_loss_in_fitting_table(fitting_entry: Dictionary, diam_in: float) -> LossLookupResult:
	var loss_lut := fitting_entry[&'loss_by_diam'] as Array
	var search_col := 0 # diam_in
	var return_col := 1 # loss
	var res := LUT_Utils.lerp_lookup(loss_lut, diam_in, search_col, return_col)
